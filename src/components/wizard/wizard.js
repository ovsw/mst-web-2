...
class Wizard extends React.Component {
  ...
  filteredCollected() {
    ...
    Object.keys(collectedTrueKeys).map((key) => {
      // eg key = performanceType
      // then loop through all their tags
      for (const keyy in this.state.passingTags[key]) {
        // eg keyy = interactive
  ...
  multiPropsFilter(performances, filters) {
    ...
    return performances.filter((performance) => {
      ...
      return filterKeys.every(
        // ... we'll test if all the active filter criteria is found it that performance's attributes
        (key) => {
          // eg = performanceType - check each filter group
          ...
          if (Array.isArray(performance[key])) {
            // performance.performanceType = ['interactive', 'game']
            ...
            return performance[key] // e.g. performance.performanceType
              .some(
                (
                  attributeValue // e.g. 'interactive'
                ) =>
    ...
    return (
      <Container>
        ...
        <PerformancesContainer
          performances={this.filterPerformances()}
          totalNoOfPerformances={performances.length}
          userInputContainerClicked={userInputContainerClicked}
        />
      </Container>
    );
  }
